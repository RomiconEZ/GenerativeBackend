...
async def create_task_for_answer_generation(
    ...
    if context:
        job = await queue.pool.enqueue_job(
            "async_gen_ans_using_llm", {"context": context}
        )  # type: ignore
...
async def create_task_for_audio_generation(
    ...
    if text.text:
        job = await queue.pool.enqueue_job(
            "async_gen_audio_for_text", {"text": text.text, "customer_id": customer.id}
        )  # type: ignore
        return {"id": job.job_id}
    else:
        return None
...
async def get_task_for_answer_generation(task_id: str) -> str:
    ...
    if getattr(job_info, "success", False):
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Task not completed or failed."
        )
...
async def get_task_for_audio_generation(
    ...
    if getattr(job_info, "success", False):
        ...
        try:
            return FileResponse(
                path_to_gen_audio, media_type="audio/ogg", filename="generated_audio.ogg"
            )
        except FileNotFoundError:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
